...
func getAreaUnit(w http.ResponseWriter, r *http.Request) (apperr *util.AppError) {
	var req httpserver.Request
	req.InitCheckOss(r)
	uid := req.GetParamInt("uid")
	seq := req.GetParamInt("seq")
	...
	uuid := util.GenUUID()
	resp, rpcerr := httpserver.CallRPC(util.AdvertiseServerType, uid, "FetchAreaUnit",
		&common.CommRequest{
			Head: &common.Head{Uid: uid, Sid: uuid},
			Seq:  seq,
			Num:  num,
		})
	httpserver.CheckRPCErr(rpcerr, "FetchAreaUnit")
	res := resp.Interface().(*advertise.AreaUnitReply)
	httpserver.CheckRPCCode(res.Head.Retcode, "FetchAreaUnit")
...
func getAdClick(w http.ResponseWriter, r *http.Request) (apperr *util.AppError) {
	...
	uuid := util.GenUUID()
	resp, rpcerr := httpserver.CallRPC(util.AdvertiseServerType, uid, "FetchAdRecords",
		&common.CommRequest{
			Head: &common.Head{Uid: uid, Sid: uuid},
			Seq:  seq,
			Num:  num,
		})
	httpserver.CheckRPCErr(rpcerr, "FetchAdRecords")
	res := resp.Interface().(*advertise.AdRecordsReply)
	httpserver.CheckRPCCode(res.Head.Retcode, "FetchAdRecords")
